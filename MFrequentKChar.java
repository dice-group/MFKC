...
public class MFrequentKChar {
	...
	public static String mostOcurrencesElement(char[] array, int k) {
		...
		//System.out.println("MFKC: " + output);
		return output;
	}
		...
		for (int i = 0; i < str1.length(); i = k) {
			...
			if (Character.isLetter(str1.charAt(i))) {
				...
				if (pos >= 0) {
					...
					similarity += formatNumbers(digitStr2)
							+ formatNumbers(digitStr1);
	...
	public static int SDFfunc(String str1, String str2, int limit) {
		return getDiff(mostOcurrencesElement(str1.toCharArray(), 2), mostOcurrencesElement(str2.toCharArray(), 2),
				limit);
	}

	private static int formatNumbers(String pString) {
		return Integer.parseInt(pString.replaceAll("[^\\d.]", ""));
	}
}
